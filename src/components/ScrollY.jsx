...
export default function ScrollY() {
    ...
    //     const init = () => {
    //         // rAF ensures layout is fully painted before we read scrollWidth
    //         requestAnimationFrame(() => {
    //             ScrollTrigger.refresh();

    //             const totalScroll = content.scrollWidth - content.clientWidth;

    //             // get the real navbar height so pinning starts below it
    //             const navHeight = document.querySelector('nav')?.offsetHeight ?? 60;

    //             st = ScrollTrigger.create({
    //                 trigger: section,
    //                 start: `top ${navHeight}px`,       // ← accounts for sticky navbar
    //                 end: `+=${totalScroll}`,
    //                 pin: true,
    //                 pinSpacing: true,
    //                 scrub: 1,
    ...
    return (
        <>
            ...
            {/* full section — fixed height so ScrollTrigger can pin reliably */}
            <section className="h-[600px] bg-primary-black flex justify-center relative">

                {/* outer window — h-full works now because parent has fixed height */}
                <div
                    ref={containerRef}
                    ...
                    className="h-full w-full max-w-[1024px] bg-primary-black overflow-x-scroll [scrollbar-width:none] [&::-webkit-scrollbar]:hidden will-change-[scroll-position]"
                >
                    {/* inner flex card holder */}
                    ...
                    {cards.map((_, i) => (
                        <button 
                            ...
                            style={{
                                ...
                                transition: 'width 350ms cubic-beizer(0.34, 1.56, 0.64, 1), opacity 300ms ease'
                            }}
                            className='rounded-full bg-primary-white shrink-0 border-none outline-none cursor-pointer'
                        />
                    ))}
                </div>
